...
class JointController(Node):
    def __init__(self):
        ...
        self.target_positions = self.initial_positions
...
def main(args=None):
    ...
    # Loop to alternate between second and third positions 4 times
    for _ in range(4):
        # Send second positions
        ...
        # Send third positions
        joint_controller.send_trajectory_command(joint_controller.third_positions)

        # Loop until all joints have arrived at the third positions
        while rclpy.ok():
            rclpy.spin_once(joint_controller)
            if all(joint_controller.is_arrived.values()):
                break
